...
#callout[*선택 가이드.* 채팅으로 해결되면 ChatGPT/Claude 웹. 코드 파일을 만지면 Codex 또는 Claude Code. 비개발자 (글로벌경영 3인) 는 웹 채팅으로 충분.]

=== 프롬프팅 5원칙 (어떤 도구든 공통)

+ *역할 부여* — "너는 LG전자 상품기획 10년차 매니저야." 톤·관점이 즉시 잡힌다.
+ *맥락 (context) 명시* — 우리가 뭘 하는 중인지·산출물 누가 보는지. 부족하면 일반론으로 회귀.
+ *구체적 출력 형식* — "표로", "5개 bullet", "300자 이내", "Markdown". 형식을 안 정하면 매번 결과가 흔들린다.
+ *예시 1\~2개 (few-shot)* — 원하는 결과를 직접 보여주는 게 설명보다 빠르다.
+ *검증·재시도* — 첫 답은 초안. "이 중 출처가 약한 항목을 표시해줘" 같은 follow-up 으로 깎는다.
...
+ *민감 정보 (개인정보·내부 자료) 를 무료 채팅에 붙여넣지 말 것* — 학습 데이터로 흘러갈 수 있다. 멘토 미팅 내용·LG 내부 자료는 절대 금지.
+ *AI 출력을 그대로 발표 자료에 박지 말 것* — 통계·인용은 반드시 출처 확인. AI 는 자신 있게 틀린 숫자를 만든다 (할루시네이션).
+ *"AI 가 알아서 해주겠지" 금지* — 프롬프트가 막연하면 결과도 막연하다. 5원칙 체크리스트로 자기 프롬프트 깎고 시작.
